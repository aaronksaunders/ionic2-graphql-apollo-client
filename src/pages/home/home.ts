...
export class HomePage implements OnInit {
  ...
  ngOnInit() {
    // this.allUsers =
    try {
      this.apollo.watchQuery({ query: AllUsers }).valueChanges;
    } catch (e) {
      console.log(e);
    }
    ...
    // this.allCompanies = this.apollo.watchQuery({
    //   query: AllCompanies
    // }).valueChanges;
  }
      ...
      .mutate({
        ...
        updateQueries: {
          AllUsers: (prev, { mutationResult }) => {
            ...
            return {
              allUsers: prevAllUsers.filter(u => {
  ...
  addUserClicked(_formValue) {
    ...
    let params = Object.assign(_formValue, {
      companyId: _formValue.company.id
    });
